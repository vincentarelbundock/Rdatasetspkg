...
expect_error(rdocs(123), pattern = "must be a character string")
expect_error(rdocs("iris", 123), pattern = "must be a character string")

# Test error for nonexistent dataset
expect_error(rdocs("nonexistent_dataset_xyz"), pattern = "not found")

# Test error for nonexistent package + dataset combination
expect_error(rdocs("nonexistent_dataset", "datasets"), pattern = "not found in package")

# Test error for nonexistent package
expect_error(rdocs("iris", "nonexistent_package"), pattern = "Package .* not found in Rdatasets")
